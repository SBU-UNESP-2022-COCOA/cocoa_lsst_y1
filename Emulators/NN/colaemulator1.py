...
def get_boost(cosmo_params, ks = emu.ks_default_precision, z = emu.redshifts):
    ...
    boost_in_cola_ks_zs = np.zeros((len(emu.redshifts), len(emu.ks_default_precision)))
    ...
    if np.array_equal(ks, emu.ks_default_precision) and np.array_equal(z, emu.redshifts):
        ...
        boost_interpolator = interp2d(emu.ks_default_precision, emu.redshifts, boost_in_cola_ks_zs)
