...
for i in range(len(emu.redshifts)):
    nn_models.append(keras.models.load_model(f'./NN_MODELS_Q/COLA_EMU_2_NHID=2_NEURONS=1024_z={emu.redshifts[i]:.3f}'))
        
###### Loading mins and maxs for rescaling normalized log boosts and PC components
mins, maxs = np.loadtxt('./Data/mins_maxs_high_precision.txt', unpack=True)
mins_pcs, maxs_pcs = np.loadtxt('./Data/mins_maxs_pcs_high_precision.txt', unpack=True)
...
for i in range(len(emu.redshifts)):
    averages[i] = np.loadtxt('./Data/Averages/average_high_precision_{:.2f}.txt'.format(emu.redshifts[i]))

###### Loading PC bases for all redshifts
principal_components = np.zeros((len(emu.redshifts), 6, len(emu.ks_high_precision)))
for i in range(len(emu.redshifts)):
    principal_components[i,:,:] = np.loadtxt('./Data/PC_Basis/components_high_precision_{:.2f}.txt'.format(emu.redshifts[i]))
