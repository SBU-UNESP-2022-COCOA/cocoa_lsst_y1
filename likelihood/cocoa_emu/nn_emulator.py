...
class NNEmulator:
    ...
    def train(self, X, y, test_split=None, batch_size=32, n_epochs=100):
        ...
        X_test = torch.Tensor(np.load('./projects/lsst_y1/emulator_output/emu_test/test_samples.npy'))
        y_test = torch.Tensor(np.load('./projects/lsst_y1/emulator_output/emu_test/test_data_vectors.npy')[:,:self.output_dim])
        X_test = (X_test - self.X_mean) /self.X_std

        # testset = torch.utils.data.TensorDataset(X_test, y_test)
        # testloader = torch.utils.data.DataLoader(tset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=1)
        ...
        for _ in epoch_range:
            ...
            y_test_pred = self.model(X_test) * self.dv_max
            # print("y_test_pred:", y_test_pred)
            # print("y_test: ", y_test)
            # quit()
            delta_y = (y_test - y_test_pred) 
            loss_test = torch.mean(torch.abs((delta_y @ self.cov_inv) @ torch.t(delta_y) ))
            losses_test.append(loss_test.detach().numpy())


                
            epoch_range.set_description('Loss: {0}, Loss_test: {1}'.format(loss,loss_test))
        
        np.savetxt("losses.txt", np.array([losses,losses_test]), fmt='%s')
        np.savetxt("test_dv.txt", np.array( [y_test.detach().numpy()[-1], y_test_pred.detach().numpy()[-1]] ), fmt='%s')
        self.trained = True
