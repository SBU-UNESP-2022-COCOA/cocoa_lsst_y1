...
class multi_gaussian(Likelihood):
    """
    Gaussian likelihood.
    """
    file_base_name = 'multi_gaussian'
    ...
    def read_covs(self, covfile):
        ...
        for i in range(self.covs_dim):
            ...
            cov.append(line[:self.covs_dim])
        cov = np.array(cov)
        return cov

    def random_SO_N(self, dim, random_state):
        """
    ...
    def covs_rotation(self, cov):
        R = self.random_SO_N(self.covs_dim, np.random)
        return np.transpose(R) @ cov @ R

    def covs_diag(self, cov):
        return np.diag(np.sqrt(np.diag(cov)))

    def means_rotation(self, means):
        #R = self.random_SO_N(self.covs_dim, np.random)
        R = special_ortho_group.rvs(self.covs_dim,)
        return means @ R
    ...
    def initialize_with_params(self):
        ...
        cov = self.read_covs(self.covs_file)
        ###rotation
        if self.random_rotation ==True:
            print("rotating input covariance matrix")
            cov = self.covs_rotation(cov)
        #self.means = self.means_rotation(self.means)

        ###
        if self.cov_diag ==True:
            print("diagonalizing input covariance matrix")
            cov = self.covs_diag(cov)


        #Temperature: here is cov not inv_cov, so should multiply T
        cov = cov * self.temperature
