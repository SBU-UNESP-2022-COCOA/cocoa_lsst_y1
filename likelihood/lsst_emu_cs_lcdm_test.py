...
import sys
import numpy as np
# from cobaya.likelihoods.cocoa_emu import Config, CocoaModel, NNEmulator, GPEmulator
# from cobaya.likelihoods.cocoa_emu.sampling import EmuSampler
import emcee
import os
...
class lsst_emu_cs_lcdm(Likelihood):
    ...
    def compute_datavector(self, theta):        
        if(self.emu_type=='nn'):
            ...
            print("GP emulator not supported for now")
            exit()
    ...
    def logp(self, **params_values):
        ...
        print("theta = ",theta)
        print("fid_dv = ", self.dv_obs, 'with shape', np.shape(self.dv_obs), self.dv_obs[-10:])
